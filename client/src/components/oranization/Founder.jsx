...
function Founder() {
  ...
  const fetchItems = async () => {
    const response = await axios.get('http://localhost:3000/organization/founder');
  ...
  const handleAdd = async () => {
    ...
    try{
       ...
       await axios.post('http://localhost:3000/organization/founder',data , {
        headers: {
  ...
  const handleUpdate = async () => {
    ...
    try {
      await axios.put(`http://localhost:3000/organization/founder/${editIndex}`, data, {
        headers: {
          'Content-Type': 'multipart/form-data',
        },
      });
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`http://localhost:3000/organization/founder/${id}`);
          ...
          {items.map((item, index) => (
            <tr key={item._id}>
              ...
              <td>{item.description.length>120 ? item.description.substring(0, 120) + "..."
                      : item.description}</td>
              <td>  <img src={`http://localhost:3000/founder/${item.image.split('/').pop()}`} alt="Item" style={{ width: '80px' }} /></td>
              <td>
