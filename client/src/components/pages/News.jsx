...
function News() {
  ...
  const handleUpdate = async () => {
    ...
    try {
      await axios.put(`http://localhost:3000/api/news/${editIndex}`, data, {
        headers: {
          'Content-Type': 'multipart/form-data',
        },
      });
          ...
          {items.map((item, index) => (
            <tr key={item._id}>
              ...
              <td>{item.description.length>120 ? item.description.substring(0, 120) + "..."
                      : item.description}</td>
              <td>  <img src={`http://localhost:3000/news-image/${item.image.split('/').pop()}`} alt="Item" style={{ width: '80px' }} /></td>
              <td>
