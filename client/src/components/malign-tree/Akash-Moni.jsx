...
function AkashMoni() {
  ...
  const handleAdd = async () => {
    ...
    try{
       ...
       await axios.post('http://localhost:3000/malign/akash-moni',data , {
        headers: {
  ...
  const handleUpdate = async () => {
    ...
    try {
      await axios.put(`http://localhost:3000/malign/akash-moni/${editIndex}`, data, {
        headers: {
          'Content-Type': 'multipart/form-data',
        },
      });
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`http://localhost:3000/malign/akash-moni/${id}`);
          ...
          {items.map((item, index) => (
            <tr key={item._id}>
              ...
              <td>{item.description.length>120 ? item.description.substring(0, 120) + "..."
                      : item.description}</td>
              <td>  <img src={`http://localhost:3000/malign-tree/akash-moni/${item.image.split('/').pop()}`} alt="Item" style={{ width: '80px' }} /></td>
              <td>
